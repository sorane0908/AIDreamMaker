  ...
  const renderSettingsTab = () => (
    <>
      {renderSection("APIキー設定", "AI機能を利用するには、ご自身のGoogle AI APIキーが必要です。",
        <>
            <p className="text-sm text-slate-600 -mt-3 mb-4">ブラウザに保存されます。</p>
            <div className="flex flex-col sm:flex-row gap-2 items-start">
                <div className="relative flex-grow w-full">
                    ...
                    <input
                        ...
                        className="w-full p-2 pl-10 bg-white border border-gray-300 rounded-lg focus:ring-2 focus:ring-sky-400"
                    />
                </div>
                ...
                <a href="https://aistudio.google.com/app/apikey" target="_blank" rel="noopener noreferrer" className="text-sm text-center sm:text-left text-sky-600 hover:underline pt-2 sm:pt-2.5 w-full sm:w-auto whitespace-nowrap">APIキーを取得</a>
            </div>
            <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    <label htmlFor="model-select" className="block text-sm font-medium text-slate-700">AIモデル</label>
                    <p className="text-xs text-slate-500 mb-1">物語生成の品質と速度のバランスを選択します。</p>
                    <select
                        id="model-select"
                        value={selectedModel}
                        onChange={(e) => setSelectedModel(e.target.value as StoryModel)}
                        className="w-full p-2 mt-1 bg-white border border-gray-300 rounded-lg focus:ring-2 focus:ring-sky-400"
                    >
                ...
                <div>
                    <label htmlFor="thinking-budget" className="block text-sm font-medium text-slate-700">
                    ...
                    <input
                        ...
                        className="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer accent-sky-500 mt-2"
                    />
              ...
              <div className="flex flex-wrap gap-x-6 gap-y-2 mt-2">
                {(['short', 'normal', 'long'] as StoryLength[]).map(len => (
                  <div key={len} className="flex items-center">
                    ...
                    />
                    <label htmlFor={`${formId}-length-${len}`} className="ml-2 block text-sm text-slate-900">
                      {len === 'short' && '短い (約250字)'}
                      {len === 'normal' && '普通 (約500字)'}
                      {len === 'long' && '長い (約800字)'}
                    </label>
      ...
      {renderSection("キャラクター", "物語に登場する人物を設定します。最初のキャラクターが主人公（一人称視点）となります。",
        <div className="space-y-6">
          {characters.map((char, index) => (
            <div key={char.id} className="border-t border-sky-200 pt-6">
                ...
                {(['name', 'gender', 'age'] as const).map(field => {
                    ...
                    return (
                        <div key={field}>
                            <label htmlFor={`${formId}-${char.id}-${field}`} className="block text-sm font-medium text-slate-700">{labels[field]}</label>
                            <input
                                ...
                                className="w-full mt-1 p-2 bg-white border border-gray-300 rounded-lg focus:ring-2 focus:ring-sky-400"
                            />
                        </div>
                    )
                })}
                <div className="md:col-span-2">
                    <label htmlFor={`${formId}-${char.id}-personality`} className="block text-sm font-medium text-slate-700">性格</label>
                    <textarea id={`${formId}-${char.id}-personality`} value={char.personality} onChange={(e) => handleCharacterChange(char.id, 'personality', e.target.value)} placeholder="例：冷静沈着だが、仲間思い" className="w-full mt-1 p-2 bg-white border border-gray-300 rounded-lg focus:ring-2 focus:ring-sky-400" rows={2}></textarea>
                </div>
                <div className="md:col-span-2">
                    <label htmlFor={`${formId}-${char.id}-ability`} className="block text-sm font-medium text-slate-700">能力・特技</label>
                    <textarea id={`${formId}-${char.id}-ability`} value={char.ability} onChange={(e) => handleCharacterChange(char.id, 'ability', e.target.value)} placeholder="例：高速な剣技、古代魔法の知識" className="w-full mt-1 p-2 bg-white border border-gray-300 rounded-lg focus:ring-2 focus:ring-sky-400" rows={2}></textarea>
                </div>

                <div className="md:col-span-2">
                    <label htmlFor={`${formId}-${char.id}-freeText`} className="block text-sm font-medium text-slate-700">自由記述欄</label>
                    <p className="text-xs text-slate-500 mb-1">背景、人間関係、口調など、その他の設定を自由に記述してください。</p>
                    <textarea id={`${formId}-${char.id}-freeText`} value={char.freeText} onChange={(e) => handleCharacterChange(char.id, 'freeText', e.target.value)} placeholder="例：王家の生き残りであることを隠している。皮肉屋な口調で話すことが多い。" className="w-full mt-1 p-2 bg-white border border-gray-300 rounded-lg focus:ring-2 focus:ring-sky-400" rows={3}></textarea>
                </div>
                ...
                <div className="md:col-span-2 flex items-center justify-between">
                    <div className="flex items-center gap-2">
                        ...
                        />
                        <label htmlFor={`${formId}-${char.id}-isOriginal`} className="text-sm text-slate-700">オリジナルキャラクター</label>
                    </div>
               ...
               {index > 0 && <button onClick={() => handleRemoveCharacter(char.id)} className="text-red-500 hover:text-red-700 text-sm md:col-span-2 mt-2">このキャラクターを削除</button>}
            </div>
            </div>
          ))}
      ...
      {renderSection("リサーチ", "物語の解像度を上げるため、原作やキャラクターの情報をAIに調査させます。（任意）",
        <div className="space-y-4">
            <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">原作の世界観・あらすじ</label>
                <div className="flex gap-2">
                    <input type="text" value={researchSourceTopic} onChange={e => setResearchSourceTopic(e.target.value)} placeholder="例：「作品名」" className="flex-grow p-2 border rounded-lg"/>
                 ...
                 {researchSourceResult && (
                    <div className="mt-2 p-2 bg-sky-100/50 rounded-md text-sm text-slate-700 flex justify-between items-center">
                        <span className="truncate pr-4"><strong>調査結果:</strong> {researchSourceResult.text.substring(0, 80)}...</span>
                        <button onClick={() => handleOpenResearchEditor('source')} className="flex-shrink-0 ml-2 px-3 py-1.5 text-xs bg-white border border-sky-300 text-sky-800 rounded-lg hover:bg-sky-100 flex items-center gap-1 transition-colors">
            ...
            <div className="border-t pt-4">
                 <label className="block text-sm font-medium text-slate-700 mb-1">特定キャラクターの背景</label>
                 <p className="text-xs text-slate-500 mb-2">原作名を入力すると、キャラクター名と組み合わせて検索し、精度を高めます。</p>
                <div className="flex gap-2">
                    <input type="text" value={researchCharacterTopic1} onChange={e => setResearchCharacterTopic1(e.target.value)} placeholder="キャラクター名（例：登場人物A）" className="flex-grow p-2 border rounded-lg"/>
                ...
                {researchCharacterResult1 && (
                    <div className="mt-2 p-2 bg-sky-100/50 rounded-md text-sm text-slate-700 flex justify-between items-center">
                        ...
                        <button onClick={() => handleOpenResearchEditor('character1')} className="flex-shrink-0 ml-2 px-3 py-1.5 text-xs bg-white border border-sky-300 text-sky-800 rounded-lg hover:bg-sky-100 flex items-center gap-1 transition-colors">
                            <PencilIcon className="w-3 h-3" />
                            確認・編集
                        </button>
                    </div>
            ...
            <div>
                <div className="flex gap-2 mt-2">
                    <input type="text" value={researchCharacterTopic2} onChange={e => setResearchCharacterTopic2(e.target.value)} placeholder="キャラクター名（例：登場人物B）" className="flex-grow p-2 border rounded-lg"/>
                ...
                {researchCharacterResult2 && (
                    <div className="mt-2 p-2 bg-sky-100/50 rounded-md text-sm text-slate-700 flex justify-between items-center">
                        ...
                        <button onClick={() => handleOpenResearchEditor('character2')} className="flex-shrink-0 ml-2 px-3 py-1.5 text-xs bg-white border border-sky-300 text-sky-800 rounded-lg hover:bg-sky-100 flex items-center gap-1 transition-colors">
                            <PencilIcon className="w-3 h-3" />
                            確認・編集
                        </button>
                    </div>
      ...
      {renderSection("データの管理", "現在の設定をファイルに保存したり、ファイルから読み込んだりします。",
        <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <button onClick={triggerImport} className="flex items-center justify-center gap-2 px-4 py-3 bg-white border border-sky-300 text-sky-800 rounded-lg hover:bg-sky-100 transition-colors">
                    <UploadIcon className="w-5 h-5" />
                    <span>設定を読込</span>
                </button>
                <button onClick={handleExportSettings} className="flex items-center justify-center gap-2 px-4 py-3 bg-white border border-sky-300 text-sky-800 rounded-lg hover:bg-sky-100 transition-colors">
                    <DownloadIcon className="w-5 h-5" />
                    <span>設定を保存</span>
                </button>
            </div>
            <button onClick={handleResetState} className="w-full flex items-center justify-center gap-2 px-4 py-3 bg-red-50 border border-red-300 text-red-800 rounded-lg hover:bg-red-100 transition-colors">
                <span>最初からやり直す</span>
            </button>
        </div>
      )}
      ...
      <div className="mt-8 text-center">
        ...
        <div className="mt-4 text-sm text-sky-800 bg-sky-100/80 p-4 rounded-lg text-left font-sans leading-relaxed">
    ...
    <div className="p-2 sm:p-4 relative pb-24">
        ...
        {storyHistory.length === 0 ? (
            <div className="text-center p-8 bg-white/50 rounded-lg">
                <p className="text-slate-600">「設定」タブで物語の準備を整え、「物語を紡ぎ始める」ボタンを押してください。</p>
            </div>
        ) : (
            <>
                <div className={`bg-white/60 p-3 sm:p-6 rounded-lg leading-relaxed min-h-[300px] ${isMultiDeleteMode ? 'cursor-pointer' : ''}`}>
                    {storyHistory.map((part, index) => {
                        ...
                        const content = isLastSegment && isEditingLastSegment && !isMultiDeleteMode
                            ? <textarea key={`text-${index}`} value={part} onChange={(e) => handleEditLastSegment(e.target.value)} className="w-full h-48 p-2 bg-white/80 border border-sky-300 rounded-lg focus:ring-2 focus:ring-sky-400" />
                            : <p key={`p-${index}`} className="whitespace-pre-wrap text-slate-800">{part}</p>;
                        ...
                        return (
                            <div 
                                ...
                                className={`
                                    ...
                                    ${isMultiDeleteMode ? 'hover:bg-sky-100' : ''}
                                    ${isSelectedForDelete ? 'bg-red-100 border border-red-300 opacity-70' : ''}
                                `}
                            >
                                {index > 0 && <hr className="my-6 border-sky-200" />}
                                {content}
                                {isSelectedForDelete && (
                                    <p className="text-red-600 text-xs font-bold text-center mt-2">削除対象</p>
                                )}
                            </div>
                        );
                    })}
                ...
                {isMultiDeleteMode && (
                    <div className="mt-6 p-4 bg-red-100 border border-red-300 rounded-lg flex justify-between items-center sticky bottom-4 shadow-lg">
                        ...
                        <div className="flex gap-2">
                            <button onClick={handleCancelMultiDelete} className="px-4 py-2 bg-white text-slate-700 rounded-lg border border-slate-300 hover:bg-slate-50 text-sm">
                                キャンセル
                            </button>
                             <button 
                        ...
                        {isEditingLastSegment ? (
                           ...
                           <div className="flex flex-col gap-4">
                               <textarea
                                    ...
                                    className="w-full flex-grow p-3 bg-white border border-gray-300 rounded-lg focus:ring-2 focus:ring-sky-400"
                                    rows={3}
                                />
                                {suggestions.length > 0 && !isLoading && (
                                    <div className="pt-2">
                                        <p className="text-sm font-medium text-slate-600 mb-2">次の展開のヒント:</p>
                                        <div className="flex flex-wrap gap-2">
                                            {suggestions.map((s, i) => (
                                                <button key={i} onClick={() => handleSuggestionClick(s)} className="px-3 py-1.5 bg-sky-100 text-sky-800 text-sm rounded-full hover:bg-sky-200 transition-colors">
                                                    {s}
                                                </button>
                                            ))}
                                        </div>
                                ...
                                <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                                    ...
                                    <div className="relative md:col-span-4">
                                        <button 
                                            ...
                                            className={`w-full px-6 py-3 border rounded-lg shadow-sm flex justify-center items-center gap-2 transition-colors ${isMenuOpen ? 'bg-slate-200 text-slate-800 border-slate-400' : 'bg-slate-100 text-slate-700 border-slate-300 hover:bg-slate-200'}`}
                                        >
                                        ...
                                        {isMenuOpen && (
                                            <div className="absolute bottom-full right-0 w-[calc(200%+0.5rem)] md:left-0 md:right-auto md:w-full mb-3 bg-white rounded-xl shadow-2xl border border-slate-200 overflow-hidden z-20 animate-fade-in-up origin-bottom">
                                                ...
                                                <div className="p-4 bg-white">
                                                    <div className="text-sm font-bold font-serif text-slate-700 flex items-center gap-2 mb-2">
                                                        <SparklesIcon className="w-4 h-4 text-sky-500"/>
                                                        <span>アクション</span>
                                                    </div>
                                                    <div className="grid gap-2">
                                                        <button onClick={handleJustSuggest} className="w-full text-left px-4 py-3 bg-sky-50 hover:bg-sky-100 border border-sky-100 rounded-lg text-sky-900 text-sm transition-colors flex items-center gap-3">
                                                    ...
                                                    <div className="text-sm font-bold font-serif text-slate-700 flex items-center gap-2 mt-4 mb-2">
                                                        <TrashIcon className="w-4 h-4 text-red-500"/>
                                                        <span>削除とやり直し</span>
                                                    </div>
                                                    <div className="grid gap-2">
                                                        <button onClick={handleDeleteAndSuggest} className="w-full text-left px-4 py-3 bg-red-50 hover:bg-red-100 border border-red-100 rounded-lg text-red-900 text-sm transition-colors flex items-center gap-3">
                                                        ...
                                                        <div className="grid grid-cols-2 gap-2">
                                                            <button onClick={handleDeleteSimple} className="text-left px-3 py-2 hover:bg-slate-100 border border-slate-200 rounded-lg text-slate-600 text-xs transition-colors">
                                                                直前を削除のみ
                                                            </button>
                                                            <button onClick={initiateMultiDelete} className="text-left px-3 py-2 hover:bg-slate-100 border border-slate-200 rounded-lg text-slate-600 text-xs transition-colors">
                                                                複数選択して削除
                                                            </button>
                                                        </div>
                                                    </div>
                                                ...
                                                <div className="p-4 bg-slate-50 border-t border-slate-200">
                                                    <p className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-3">クイック設定 (即時反映)</p>
                                                    
                                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                                                        <div>
                                                            <label className="block text-xs font-medium text-slate-600 mb-1">AIモデル</label>
                                                            <select
                                                                value={selectedModel}
                                                                onChange={(e) => setSelectedModel(e.target.value as StoryModel)}
                                                                onClick={(e) => e.stopPropagation()}
                                                                className="w-full p-2 text-sm bg-white border border-slate-300 rounded-lg focus:ring-2 focus:ring-sky-400 transition-shadow"
                                                            >
                                                                <option value="gemini-2.5-flash">gemini-2.5-flash (高速)</option>
                                                                <option value="gemini-2.5-pro">gemini-2.5-pro (高品質)</option>
                                                            </select>
                                                    ...
                                                    <div>
                                                        <label className="block text-xs font-medium text-slate-600 mb-1">物語の方向性</label>
                                                        <textarea
                                                            value={storyDirection}
                                                            onChange={(e) => setStoryDirection(e.target.value)}
                                                            onClick={(e) => e.stopPropagation()}
                                                            className="w-full h-32 p-2 text-sm bg-white border border-slate-300 rounded-lg focus:ring-2 focus:ring-sky-400 transition-shadow"
                ...
                {storyHistory.length > 0 && !isMultiDeleteMode && (
                  <div className="mt-8 border-t border-sky-200 pt-6 flex justify-center">
                    <button onClick={handleExportStory} className="flex items-center justify-center gap-2 px-4 py-3 bg-white border border-sky-300 text-sky-800 rounded-lg hover:bg-sky-100 transition-colors">
                        <DownloadIcon className="w-5 h-5" />
                        <span>物語を保存</span>
                    </button>
                  </div>
    ...
    <div className="min-h-screen bg-gradient-to-br from-sky-50 to-blue-100 text-slate-800 p-2 sm:p-6 md:p-8">
      <div className="max-w-4xl mx-auto">
        <header className="text-center mb-8">
          ...
          <p className="mt-2 text-slate-600">思う存分推しと楽しもう！</p>
          <p className="mt-2">
            <a href="https://ameblo.jp/soranowoto0908/entry-12943949448.html" target="_blank" rel="noopener noreferrer" className="text-sky-600 hover:underline font-medium">
              📖 使い方ガイド
            </a>
          </p>
        </header>

        <div className="bg-white/70 backdrop-blur-md border border-sky-200/80 rounded-xl shadow-2xl">
          <div className="p-2 sm:p-4 border-b border-sky-200/80">
            {renderTabNavigation()}
          </div>
          ...
          {error && <div className="px-4 sm:px-6 pb-4">
            <p className="text-red-700 bg-red-100 p-4 rounded-lg whitespace-pre-wrap text-sm text-left">{error}</p>
          </div>}
          
          <div className="p-2 sm:p-4 border-t border-sky-200/80">
            {renderTabNavigation()}
          </div>
        </div>
      ...
      {showScrollToBottomButton && (
        <button
          ...
          className="fixed bottom-6 right-6 z-50 bg-sky-500 text-white rounded-full p-3 shadow-lg hover:bg-sky-600 focus:outline-none focus:ring-2 focus:ring-sky-400 focus:ring-offset-2 transition-transform transform hover:scale-110"
